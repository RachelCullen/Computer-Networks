...
/*-----------------各个全局变量定义-----------------------------------*/
const int MSS = 1024;
const int MAXSIZE = 2 * MSS;
SOCKADDR_IN server_addr;
SOCKET server;
//int SEQ = 0;
//int ACK = 0;
const long long TIMEOUT = 0.04;
double INTERACT_TIME = 1.0;
int SEND_WIND_SIZE = 12; //窗口大小
int ssthreash = 20; //阈值
    ...
    while (base < num - 1) {//前整个maxsize的包都这么发最后一个单独发
        ...
        if (recvfrom(clientsocket, buf, MAXSIZE, 0, (sockaddr*)&serveraddr, &addrlen)) {
            ...
            if (recvh.ack >= base % 256) {
                ...
                //recvh.show_header();
                base +=1 /*(recvh.ack - base % 256 + 1)*/;
                curAcked +=1 /*(recvh.ack - base % 256 + 1)*/;
                curAcked %= 256;
                if (base != nextseqnum)
                    QueryPerformanceCounter((LARGE_INTEGER*)&head);
            ...
            //if (recvh.ack < base % 256 ) { //发生了跨窗口的情况
            //    cout << "[ACK ] packet seq:" << (int)recvh.ack << endl;
            //    //recvh.show_header();
            //    base += (256 - base % 256 + recvh.ack);
            //    curAcked += (256 - base % 256 + recvh.ack);
            //    curAcked %= 256;
            //    if (base != nextseqnum)
            //        QueryPerformanceCounter((LARGE_INTEGER*)&head);
